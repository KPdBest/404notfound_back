			...
			if (i == data['username']):
					for x in obj:
						if(x.username == data['username'] and x.password == data['password']):
							...
							latest=check_up.objects.filter(link1__id=name[0]['link__id'], status='INSERT').values('final_remark')
							if len(latest) >0:
								data1={'status':name[0]['link__name'],"id":name[0]['link__id'], "remark":latest[0]['final_remark']}
...
def dropdown(request):
	if request.method=='GET':
		if request.GET['request_type']=='drop':
			user = user_details.objects.filter().values('id','name')
			print(user)
...
def check_insert(request):
	if request.method=='POST':
		...
		insert=check_up.objects.create(final_remark=data['remark'], blood_pressure=data['blood_pressure'],sugar=data['sugar'],thyroid=data['thyroid'],medicines=data['medicines'],height=data['height'],weight=data['weight'],heart_disease=data['heart_disease'],link1=user_details.objects.get(id=data['id']))
...
def show_checkups(request):
	if request.method=='GET':
		id=request.GET['id']
		details=check_up.objects.filter(link1__id=id).values('link1__name','link1__age','link1__mobile_no','link1__blood_group','link1__email','link1__gender','link1__address','link1__pre_Health','link1__family_doctor','link1__doc_no','link1__doc_name','link1__doc_email','link1__emergency_contact','status').order_by('-status')
		if(len(details)>0):
